...
exports.login = async (req, res) => {
  try {
    logger.info(
      ...
      "login",
      "Starting execution :: Calling authentication service",
      { path: req.path, ipAddress: req.ip }
    );
    let response = await nodeFetch(
      process.env.AUTHENTICATION_SERVICE_URL + req.path,
      {
        method: "POST",
        body: JSON.stringify({
    ...
    if (response.status === "success") {
      logger.info(
        ...
        "authentication-service",
        "login",
        "Set refresh token in httpOnly secured cookie",
        {}
      );
